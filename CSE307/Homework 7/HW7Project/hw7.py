...
class Parser(tpg.Parser):
    ...
    | block/a
    | "return" expression/e ";"$ a = Return.ReturnStatement(e) $
    | variable/f "\(" "\)" ";" $ a = Functions.CallFunction(f)$
    | variable/f "\(" argumentList/l "\)" ";" $a = Functions.CallFunction(f,l) $
    | variable/f "\(" "\)" block/s $ a = Assign.AssignFunction(f,s) $
    | variable/f "\(" argumentList/l "\)" block/s $ a = Assign.AssignFunction(f,s,l) $
    | "if" "\(" expression/a "\)" statement/i "else" statement/e $ a = IfElse.IfElseStatement(a, i, e) $
    ...
    | "%"  parens/b $ a = Modulo.Modulo(a, b) $
    )* ;

    parens/a -> "\(" expression/a "\)" | literal/a
    ;

    literal/a -> variable/f "\(" "\)" $ a = Functions.CallFunction(f)$
    | variable/f "\(" argumentList/l "\)" $a = Functions.CallFunction(f,l) $
    | index/a | int/a | variable/a | str/a | list/a
    ;
    ...
    argumentList/a -> expression/a "," argumentList/b $ a = Functions.ArgumentList(a,b) $
    | expression/a $ a = Functions.ArgumentList(a)$
    ;
    """
