...
class AssignFunction(Node.Node):
    """
    Define a function
    """
    ...
    def exectute(self):
        locationSetter = self.function.location()
        arguments = self.arguments.evaluate()
        statements = self.statements.evaluate()

        locationSetter(statements, arguments)
